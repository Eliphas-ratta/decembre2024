class Hero {
    ...
    play (enemy, heroSelected, listEnemies, randomIndexEnemy) {
        let answer = this.checkUserInput("Le jouer que vous affronté a-til un nombre de billes pair ou impair? (0) - pair / (1) - impair",0 , 1);
    ...
    checkUserInput(msg, min, max){
        ...
        //récupérer la saisie
        //verifiez si c'est un chiffre compris entre min et max
        //sinon redelander ka saie 
    if(isNaN(input) || (input < min || input > max )) {
        alert("saisie incorrect choisi un chiffre entre " + min + " et " + max);
        input = prompt(msg);
...
let answerLevel = this.checkUserInput("0 - Facile / 1 - Difficile / 2 - Moyen",0 , 1);
let listLevels = [4, 12, 18];
let nbrEncounter = listLevels[answerLevel];

let answerHero = this.checkUserInput("0 - Seong Gi-hun / 1 - Kang Sae-byeok / 2 - Cho Sang-woo",0 , 1);
...
while(nbrEncounter > 0 && heroSelected.marbles > 0) {
  ...
  heroSelected.play(heroSelected, enemy, listEnemies, randomIndexEnemy);
